...
- In Next.js, use the [custom Document]. In CRA, use [`public/index.html`].
...
However, for best visual result, we highly recommend the [Inter] typeface. To
use it, just import the following CSS at your app's root, and Moai will use
Inter instead:
...
- Moai uses the [variable] version of Inter. Check browser compatibilty if you
need to support old browsers.
- If your app cannot make network request, replace "remote" with "local" in the
path above to use a bundled version of Inter. This may require [special setup].
- It is NOT recommend to use the version on Google Fonts since it lost many
[OpenType features].
...
components are exported at top level:
