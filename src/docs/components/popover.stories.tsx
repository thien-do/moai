 ...
 * This element is always rendered, and should use the `popover` param to activate the popover on users' action.
...
/**
 * In order to position the content relative to the target, Popover needs to wrap the target inside a wrapper.
 * By default, this wrapper is an HTML `div` element with a `width: fit-content`.
 * This should work for most cases, but there are also other wrappers at `Popover.targetWrappers` for other cases:
 *
 * - `inline` should be used when your `target` is an [inline][1] element, such as a `<span />`.
 * - `block` should be used when you don't want the `fit-content` width, e.g. for full-width buttons.
 *
 ...
 * This can be changed by the `placement` prop, which expects a string of [Popper.js' `placement`][1] option.
 * You can see (and try!) all available placements at the [props table][2] below.
 *
 * Note that this is more like a suggestion.
 * If there is no space in the selected placement, Popover will choose another placement, usually the opposite one,
